  ...
  return (
    ...
    >
      <div className="relative w-full max-w-md rounded-xl border border-zinc-800 bg-zinc-950 p-8 shadow-2xl">
        ...
        <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); void handleSignIn(); }}>
          <div>
            <label className="mb-1.5 block text-sm font-semibold" htmlFor="modal-email">Email</label>
            <input className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500" id="modal-email" placeholder="you@example.com" type="email" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-semibold" htmlFor="modal-password">Password</label>
            <input className="w-full rounded-lg border border-zinc-700 bg-zinc-900 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500" id="modal-password" placeholder="Password" type="password" />
          </div>
          <button className="button blue w-full" disabled={loading} type="submit">{loading ? loadingLabel : actionLabel}</button>
        </form>
