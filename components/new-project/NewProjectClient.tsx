    ...
    <div>
      <section className="hero min-h-0 px-4 py-16">
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto mb-6 flex w-fit items-center gap-2 rounded-md border border-purple-500/30 bg-zinc-950/50 px-3 py-2 text-sm font-semibold text-white backdrop-blur">{MOCK_USER.workspace}</div>
          ...
          <div className="mx-auto mt-5 flex max-w-4xl flex-wrap justify-center gap-3">
            {PROMPT_CHIPS.map((chip) => <button className="pill" key={chip} onClick={() => setPrompt(`Build ${chip} as a polished app.`)} type="button">{chip}</button>)}
          </div>
        </div>
      </section>
