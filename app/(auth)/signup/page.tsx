...
import { ArrowRight, ArrowLeft, Github, Loader2 } from "lucide-react";
...
export default function SignupPage() {
  ...
  if (view === "otp") {
    return (
      <div>
        ...
        <form className="mt-8 space-y-5" onSubmit={(e) => void handleOtpVerify(e)}>
          <div>
            <Label className="mb-1.5 block text-sm font-semibold" htmlFor="otp">Verification code</Label>
            <Input
        ...
        <div className="mt-5 flex items-center justify-between text-sm">
          <button className="font-semibold text-purple-300 hover:underline" onClick={() => setView("signup")} type="button">
            <ArrowLeft className="mr-1 inline" size={13} />Back
          </button>
          <button className="font-semibold text-slate-400 hover:text-white disabled:opacity-50" disabled={loading} onClick={() => void handleResendOtp()} type="button">
            Resend code
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-slate-400">Start building for free — no credit card required.</p>
      ...
      <form className="space-y-4" onSubmit={(e) => void handleSignUp(e)}>
        ...
        <Button className="mt-2 w-full" disabled={loading || !email || !password} type="submit">
          {loading
            ? <><Loader2 className="mr-2 animate-spin" size={16} />Creating account…</>
      ...
      <p className="mt-6 text-center text-sm text-slate-400">
        ...
        <Link className="font-semibold text-purple-300 hover:underline" href="/login">
          Sign in
        </Link>
      </p>
    </div>
